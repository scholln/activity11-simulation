...
norm_samp_1000 %>%
  summarise(values) %>%
  mutate(mean = mean(values), sd = sd(values))
...
Standard deviation is larger and mean is lower
...
```{r exponential distributed values}

Beig_rexp <- tibble(rexp(1000, rate = 0.02)
)

smol_rexp <- tibble(rexp(30, rate = 0.02))

mean(Beig_rexp$`rexp(1000, rate = 0.02)`)
mean(smol_rexp$`rexp(30, rate = 0.02)`)

sd(Beig_rexp$`rexp(1000, rate = 0.02)`)
sd(smol_rexp$`rexp(30, rate = 0.02)`)

```

Both means are in the same ball park with the smaller one being larger
SD's have a larger difference between them
...
```{r I think this should be for the function}

generate_n_samples <- function(n, x, y, ...) {
  tibble(
    Normal = rnorm(n),
    Exponential = rexp(n),
    chi_sqaure = rchisq(n, df = x),
    uniform = runif(n, min = x, max = y)
  )
...
generate_n_samples(1000, x = 1, y = 2)
...
|--------------|------------|------------|
| Normal       |-0.09452011		 0.903102         |
| Exponential  |0.9997024      0.7421076
| Chi-square   |0.9764978      1.739869	
| Uniform      |  1.519723     0.2903031
...
sample_mean(generate_n_samples(100, 1, 2))
sample_sd(generate_n_samples(100, 1, 2))
```
...
```{r We will TRY (KEYWORD) to get a nice df in this}

The_real_samples <- map_df(1:1000, ~generate_n_samples(30, x = 1, y = 2))
